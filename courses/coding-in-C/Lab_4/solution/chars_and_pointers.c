...
int main()
{
    ...
    printf("%s\n", proverb);
    // printf("%s\n", *p_proverb); // Segmentation fault: *p_proverb ist der Inhalt des Zeigers = E; da ein Integer in printf("%s") erwartet wird, wird E als Adresse interpretiert und darauf zugegriffen
    ...
    *(p_proverb + 1) = 'a'; // Pointer arithmetik
    ...
    // func_adapt("literal"); // würde ein Segmentation Fault erzeugen, da in function der Inhalt angepasst werden soll aber es sich um einen Stringliteral handelt, der auf dem statischen Speichersegment liegt, der nur readonly ist
